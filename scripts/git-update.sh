...
PEN_X=$(awk -vFS='\t' '/Digitizer Pen\tDesktop X/ {print $6-$5}' <<< "${PEN_DATA}")
PEN_Y=$(awk -vFS='\t' '/Digitizer Pen\tDesktop Y/ {print $6-$5}' <<< "${PEN_DATA}")
...
LIBWACOM_WIDTH=$(LANG=C printf '%.0f\n' ${PEN_WIDTH})
LIBWACOM_HEIGHT=$(LANG=C printf '%.0f\n' ${PEN_HEIGHT})
